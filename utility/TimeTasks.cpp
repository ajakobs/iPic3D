  ...
  if(MPIdata::get_rank()) return;
  {
    ...
    for(int e=FIELDS; e<=MOMENTS; e++)
    {
      ...
      fprintf(file, "%s_|%6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %s\n",
        ...
        comput,
        commun[e],
        commun[e]-sndrcv[e]-allred[e]-cb[e],
        sndrcv[e],
        allred[e],
        cb[e],
        //loccom[e],
    ...
    fprintf(file, "%s_|%6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %s\n",
      ...
      computtot,
      communtot,
      communtot-sndrcvtot-allredtot-cbtot,
      sndrcvtot,
      allredtot,
      cbtot,
      //loccomtot,
    ...
    fprintf(file, "%s_|time   subtask\n", reduce_mode);
    for(int e=before_report_list+1; e<NUMBER_OF_TASKS; e++)
    {
      // do not show tasks that are not executed
      double elapsed_time = tskdur[e];
      if(!elapsed_time)
        continue;

      assert_eq(stack_depth[e],0);
      fprintf(file, "%s_|%6.3f %s\n",
      reduce_mode,
      elapsed_time,
      get_taskname(e));
    }
